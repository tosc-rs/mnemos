...
macro_rules! span {
    ($level:expr, $($arg:tt)+) => {
        crate::trace::Span {
            #[cfg(any(feature = "tracing-01", loom))]
            span_01: {
                use ::mnemos_tracing::Level;
                ::mnemos_tracing::tracing_01::span!($level, $($arg)+)
            },

            #[cfg(any(feature = "tracing-02", all(test, not(loom))))]
            span_02: {
                use tracing_02::Level;
                ::mnemos_tracing::tracing_02::span!($level, $($arg)+)
            }
        }
    }
}
...
macro_rules! trace {
    ($($arg:tt)+) => {
        #[cfg(any(feature = "tracing-01", loom))]
        {
            ::mnemos_tracing::tracing_01::trace!($($arg)+)
        }

        #[cfg(any(feature = "tracing-02", all(test, not(loom))))]
        {
            ::mnemos_tracing::tracing_02::trace!($($arg)+)
        }
    };
}
...
macro_rules! debug {
    ($($arg:tt)+) => {
        #[cfg(any(feature = "tracing-01", loom))]
        {
            ::mnemos_tracing::tracing_01::debug!($($arg)+)
        }

        #[cfg(any(feature = "tracing-02", all(test, not(loom))))]
        {
            ::mnemos_tracing::tracing_02::debug!($($arg)+)
        }
    };
}
...
macro_rules! info {
    ($($arg:tt)+) => {
        #[cfg(any(feature = "tracing-01", loom))]
        {
            ::mnemos_tracing::tracing_01::info!($($arg)+)
        }

        #[cfg(any(feature = "tracing-02", all(test, not(loom))))]
        {
            ::mnemos_tracing::tracing_02::info!($($arg)+)
        }
    };
}
...
macro_rules! warn {
    ($($arg:tt)+) => {
        #[cfg(any(feature = "tracing-01", loom))]
        {
            ::mnemos_tracing::tracing_01::warn!($($arg)+)
        }

        #[cfg(any(feature = "tracing-02", all(test, not(loom))))]
        {
            ::mnemos_tracing::tracing_02::warn!($($arg)+)
        }
    };
}
...
macro_rules! error {
    ($($arg:tt)+) => {
        #[cfg(any(feature = "tracing-01", loom))]
        {
            ::mnemos_tracing::tracing_01::error!($($arg)+)
        }

        #[cfg(any(feature = "tracing-02", all(test, not(loom))))]
        {
            ::mnemos_tracing::tracing_02::error!($($arg)+)
        }
    };
}
