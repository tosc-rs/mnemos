...
use kernel::{
    drivers::{serial_mux::{PortHandle, RegistrationError, SerialMuxServer}, forth_spawnulator::SpawnulatorServer},
    mnemos_alloc::{
        containers::Box,
        heap::{MnemosAlloc, SingleThreadedLinkedListAllocator},
    },
    trace, Kernel, KernelSettings, forth::shells::graphical_shell_mono,
};
use spim::{kernel_spim1, SpiSenderServer, SPI1_TX_DONE};
use uart::D1Uart;
mod spim;
mod uart;
mod emb_display;
...
fn main() -> ! {
    ...
    k.initialize(async move {
        emb_display::SharpDisplay::register(k, 4).await.unwrap();
    }).unwrap();

    k.initialize(graphical_shell_mono(k, 400, 240, 2, 1024)).unwrap();
